...
class HomeInformationStep7 extends StatefulWidget {
  @override
  State<StatefulWidget> createState() => _Step7State();
}

class _Step7State extends State<HomeInformationStep7> {
  final Step7Bloc bloc = GetIt.instance.get<Step7Bloc>();
        ...
        listener: (context, state) {
          if (ModalRoute.of(context)?.isCurrent == true) {
            if (state is UpdateOfferSuccessState) {
              Log.d('update success 777');
              NavigationController.navigateToIBuyQAHomeStep8(context);
            } else if (state is GetOfferDetailSuccess) {
              final offer = _propzyHomeBloc.draftOffer;
              if (offer == null) {
                return;
              }
              final expected = offer.expectedTime;
              if (expected != null) {
                currentChoice = HomeDirection(expected.id, expected.name);
              }
              setState(() {});
            }
          }
              ...
              child: Padding(
                ...
                child: Column(
                  children: [
                    ...
                    SizedBox(height: 30),
                    Text(
                      'Thời gian bạn mong muốn bán nhà',
                    ...
                    SizedBox(height: 12),
                    Text(
                      'Bạn hoàn toàn chủ động nếu muốn bán trong một vài ngày hay lên kế hoạch cho vài tuần tới.',
                      textAlign: TextAlign.center,
                      style: TextStyle(
                        fontSize: 16,
                    ...
                    SizedBox(height: 40),
                    Expanded(
                      child: BlocBuilder<Step7Bloc, Step7State>(builder: (context, state) {
                        if (state is Step7Success) {
                    ...
                    PropzyHomeContinueButton(
                      ...
                      onClick: () {
                        final reachedPage = _propzyHomeBloc
                            .getReachedPageId(PropzyHomeScreenDirect.EXPECTED_TIME.pageCode);
                        final event = UpdateOfferEvent(
                          reachedPageId: reachedPage,
                          expectedTimeId: currentChoice?.id,
                        );
