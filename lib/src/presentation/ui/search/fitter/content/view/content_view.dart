...
import 'package:propzy_home/src/presentation/ui/search/fitter/content/bloc/content_bloc.dart';
...
class ContentView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    parentBloc.contentBloc = bloc;

    return BlocListener<ContentBloc, ContentState>(
      listener: (context, state) {},
      listenWhen: (previous, current) {
        if (previous is ContentLoaded && current is ContentLoaded) {
          ...
          if (isDiff) {
            ...
            parentBloc.search(contentId: index == 0 ? Content.ALL : current.data?.elementAt(index));
          }
        }
        return true;
      },
  ...
  Widget _buildItem(Content? item, ContentBloc bloc, int index) {
    return OutlinedButton(
      ...
      onPressed: () {
        bloc.add(CheckItemContentEvent(
          index: index,
          item: item,
        ));
      },
