import 'package:flutter/material.dart';
import 'package:flutter_bloc/flutter_bloc.dart';
import 'package:propzy_home/src/presentation/ui/search/fitter/bloc/search_filter_bloc.dart';

import '../advantage.dart';
import 'advantage_view.dart';

class AdvantagePage extends StatelessWidget {
  const AdvantagePage({
    Key? key,
  }) : super(key: key);

  @override
  Widget build(BuildContext context) {
    final parentBloc = context.read<SearchFilterBloc>();
    return BlocProvider(
      create: (context) => AdvantageBloc()..add(LoadAdvantageEvent(advantage: parentBloc.categorySearchRequest.advantageIds)),
      child: const AdvantageView(),
    );
  }
}
